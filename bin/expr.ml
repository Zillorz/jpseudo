...
let prefix_bp operator =
  ...
  | _ ->
      failwith
        ("Operator " ^ Lexer.show_operator operator ^ " does not support prefix")
>>>>>>> Stashed changes

let postfix_bp operator =
  match operator with Lexer.Inclusive -> Some 8 | _ -> None

let infix_bp operator =
  match operator with
  | Lexer.Eq -> Some (2, 3)
...
=======
let rec extract_atom : Lexer.token list -> expression * Lexer.token list =
 fun list ->
>>>>>>> Stashed changes
  match list with
  ...
  | other_tok :: _ ->
      failwith ("Cannot extract atom: " ^ Lexer.string_of_token_debug other_tok)
  | _ -> failwith "Cannot extract atom from nothing"

let rec flatten_comma_list : expression -> expression list =
 fun comma_list_exp ->
...
let rec parse_expr toks min_bp =
  ...
  | Seperator Lexer.OpenParen :: toks ->
      ...
      let toks =
        ensure_seperator Lexer.ClosedParen toks
  ...
  | Seperator Lexer.OpenBracket :: toks ->
      ...
      let toks =
        ensure_seperator Lexer.ClosedBracket toks
          (fun t -> "(lst) Expected ] found " ^ Lexer.string_of_token_debug t)
          "(lst) Bracket not matched"
      in
...
and parse_partial_expr lhs toks min_bp =
  ...
  | Lexer.Operator op :: toks -> (
      ...
      | Some pf_bp ->
          if pf_bp < min_bp then (lhs, toks)
          else parse_partial_expr (Unit (op, lhs)) toks min_bp
      | None -> (
          match infix_bp op with
          | Some (l_bp, r_bp) ->
              if l_bp < min_bp then (lhs, toks)
              else
...
and parse_next_expression toks = parse_expr toks 0
