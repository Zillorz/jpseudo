...
let ensure_end_of: Lexer.keyword -> Lexer.token list -> string -> Lexer.token list = fun ensure toks reason ->
  match toks with
  | (Keyword Lexer.End)::(Keyword ensure)::toks -> toks
  | _ -> failwith reason;;

let add_ast: ast -> ast list * Lexer.token list -> ast list * Lexer.token list = fun as_unit list ->
  let (ast, tokens) = list in (as_unit :: ast, tokens);;

let rec parse_ast: Lexer.token list -> ast list * Lexer.token list = fun toks ->
  match toks with
  (* removes \n and ; at front*)
  | (Seperator Lexer.Newline)::toks | (Seperator Lexer.Semicolon)::toks -> parse_ast toks
  ...
  | (Keyword Lexer.For)::toks -> (
      let (iteration, toks) = Expr.parse_next_expression toks in (* extract Y in for Y:*)
      let toks = ensure_seperator Lexer.Colon toks "expected for loop iteration statement to end with colon" in (* ensure colon *)
      let (body, toks) = parse_ast toks in (* parse the body as an ast *)
      let toks = ensure_end_of Lexer.For toks "expected for loop to end with 'end for" in (* ensure 'end for'' *)

      add_ast (For (iteration, Block body)) (parse_ast toks) (* add the for loop to the ast, parse the rest *)
  )
  ...
  | (Keyword Lexer.While)::toks -> (
      ...
      let toks = ensure_seperator Lexer.Colon toks "expected while loop condition to end with colon" in 
      let (body, toks) = parse_ast toks in
      let toks = ensure_end_of Lexer.While toks "expected while loop to end with 'end while" in
  ...
  | (Keyword Lexer.If)::t -> (
      ...
      | (Keyword Lexer.Else)::toks ->
          let toks = ensure_seperator Lexer.Colon toks "else must be followed by colon" in
          let (else_body, toks) = parse_ast toks in
          let toks = ensure_end_of Lexer.If toks "else must be closed with end if" in
