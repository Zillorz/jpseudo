type keyword = Function | For | While | If | Else | End [@@deriving show]
...
=======
let token_of_const_numeral numstr =
  match numstr with
  (* Because ocaml's int_of_string is so good, 
   we don't need to implement these cases *)
  (* Replace if we wan't more custom parsing later *)
  (* | hex when String.starts_with ~prefix:"0x" hex 
   -> ConstInt (int_of_string hex) *)
  (* | binary when String.starts_with ~prefix:"0b" binary 
   -> ConstInt (int_of_string binary) *)
  | float when String.contains float '.' -> ConstFloat (float_of_string float)
  | regular -> ConstInt (int_of_string regular)
>>>>>>> Stashed changes
...
let rec parse_mapped string map =
  match map with
  | [] -> None
  | (pfx, tok) :: t ->
...
let parse_rgx string rgx =
  match Re.exec_opt rgx string with
  | Some g ->
      let v = Re.Group.get g 0 in
...
=======
let rec parse str =
  match String.drop_first_while (fun c -> c == ' ') str with
  | str when Option.is_some (parse_mapped str operators) ->
      let tok, rem = Option.get (parse_mapped str operators) in
      token_of_operator tok :: parse rem
  | str when Option.is_some (parse_mapped str seperators) ->
...
let rec parse_function_args tokens =
  ...
  | Ident arg_name :: Operator Comma :: toks ->
      ...
      (arg_name :: args, rtoks)
  | Ident arg_name :: Seperator ClosedParen :: toks -> ([ arg_name ], toks)
  | Seperator ClosedParen :: toks -> ([], toks)
  | _ -> failwith "Invalid function args"

let string_of_token_debug t =
  match t with
...
let rec string_of_tok_list_debug l =
  match l with
  | [] -> ""
